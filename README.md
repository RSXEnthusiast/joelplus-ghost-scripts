...
There's no separate script/state directory to configure — each script writes
its seen-id file (and `--test`/cron output goes wherever you redirect it) next
to itself, wherever you've placed it.
...
9. Other variables in `.env.example` (playlist naming, fetch count, live-skipping, email segment, link ref) can be left at their defaults or customized.
10. To change the post itself beyond what `.env` covers, edit the script. ctrl+f for `Sunday Sidecar` to find the section that builds the ghost post, as this is what I titled my weekly post.
